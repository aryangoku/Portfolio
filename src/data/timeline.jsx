
const timelineData = [
  {
    type: "education",
    org: "Vidyalankar Inst. of Technology",
    logo: "/vit-logo.png",
    title: "B.E. Information Technology",
    date: "Oct 2020 – May 2024",
    cgpa: "3.62",
    highlights: [
      "Completed Honors in AIML, learning core AI concepts including machine learning, NLP, and computer vision.",
      "Published a research paper on AI/ML (see resume for details).",
      "Built diverse projects: IoT, MERN Stack, AI-driven applications.",
      "Gained hands-on experience in both software and hardware domains."
    ]
  },
  {
    type: "experience",
    org: "ACMEGRADE",
    logo: "/acmegrade-logo.png",
    title: "Full Stack Dev Trainee",
    date: "Jan 2023 – Feb 2023",
    highlights: [
      "Completed comprehensive Full Stack Development training (React.js, JavaScript, MySQL, PHP).",
      "Built a responsive e-commerce website from scratch.",
      "Collaborated with mentors to enhance UI/UX design."
    ]
  },
  {
    type: "education",
    org: "Syracuse University",
    logo: "/syracuse-logo.png",
    title: "Masters in Computer Science",
    date: "Aug 2024 – May 2026",
    cgpa: "3.44",
    highlights: [
      "Developed an AI-powered Nutrient Analysis system leveraging OCR and NLP for automated food label recognition and diet recommendations.",
      "Learned and applied Haskell for advanced problem-solving and functional programming concepts.",
      "Built a strong foundation in Operating Systems, concurrency, and systems programming.",
      "Hands-on experience with modern AI/ML tools: PyTorch, Hugging Face, and more."
    ]
  }
];

export default timelineData;
